...
namespace Memory
{
	...
	public:
		...
		uintptr_t begin() { return m_begin; }
		uintptr_t end() { return m_end; }
	...
	template <typename T>
	uintptr_t get_adjusted(T address)
	{
		return (uintptr_t)address + baseAddressDifference;
	}

	template <typename ValueType, typename AddressType>
	void put(AddressType address, ValueType value)
	{
		adjust_base(address);
	...
	template <typename ValueType, typename AddressType>
	void putVP(AddressType address, ValueType value)
	{
		adjust_base(address);
	...
	template <typename AddressType>
	void return_function(AddressType address, uint16_t stackSize = 0)
	{
		if (stackSize == 0)
		{
	...
	void set_call(TTarget* target, T address)
	{
