...
void Log::Init(HMODULE hModule)
{
	...
	if (GetModuleFileNameA(hModule, g_logFile, MAX_PATH) != 0)
	{
		...
		sprintf_s(chLogBuff, szTimestamp, "INIT: KARMA.red");
		if (slash != -1)
		{
			g_logFile[slash + 1] = '\0';
			strcpy_s(g_debugLogFile, g_logFile);
			strcat_s(g_debugLogFile, "KARMA.red");
...
void Log::Error(const char* fmt, ...)
{
	...
	MessageBoxA(nullptr, chLogBuff, "ERROR", MB_ICONERROR);
}
...
void Log::Fatal(const char* fmt, ...)
{
	...
	MessageBoxA(nullptr, chLogBuff, "FATAL ERROR", MB_ICONERROR);
