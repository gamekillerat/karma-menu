...
typedef BOOL(__cdecl*	fpCreateAmbientDrop)(DWORD pickupHash, Vector3* playerPosition, int unk0, int value, DWORD modelHash, bool unk1, bool unk2);
...
typedef uint32_t(__cdecl*	fpGetPedNearbyVehicles)(Ped ped, int* sizeAndVehs);
typedef uint32_t(__cdecl*	fpGetPedNearbyPeds)(Ped ped, int* sizeAndPeds, int ignore);
typedef Vector3(__cdecl*    fpGetEntityCoords) (Entity entity, BOOL alive);// doi this isnt in hooking
class Hooking
{
private:
...
public:
	...
	static fpClonePed							Hooking::clone_ped;
	static fpGetEntityCoords						Hooking::get_entity_coords;
	static fpStatSetBool			Hooking::stat_set_bool;
	static void						ownedExplosionBypass(bool toggle);
	static fpGetPedNearbyVehicles  Hooking::get_ped_nearby_vehicles;
	static fpCreateAmbientDrop						Hooking::create_ambient_pickup;
	static fpNetworkShopBeginService Hooking::network_shop_begin_service;
	static fpStatSetFloat Hooking::set_stat_float;
	static fpSetMobileRadio Hooking::set_mobile_radio;
